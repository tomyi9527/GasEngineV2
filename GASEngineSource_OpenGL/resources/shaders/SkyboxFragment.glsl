...
vec3 evaluateDiffuseSphericalHarmonics(const in vec3 s[9], const in vec3 n) {
 ...
 { 
	...
	vec3 uvWorldRotated = rotate_vertex_position(uvInWorldSpace, vec3(0.0, 1.0, 0.0), orientation);
		...
		FragColor = vec4(linearTosRGB(rgb, 2.4), 0.0);
	#endif
		...
		FragColor = vec4(linearTosRGB(colorRGB, 2.4), 0.0);
	#endif
#endif

#ifdef IMAGE
	vec2 uvBackgroundImage = (fragmentTexCoord0.xy*0.5)*imageRatio + vec2(0.5, 0.5);
    vec4 colorRGB = texture(image, uvBackgroundImage).rgba;
    FragColor = vec4(colorRGB.rgb, 0.0);
#endif

#ifdef SOLIDCOLOR
	FragColor = vec4(solidColor.x, solidColor.y, solidColor.z, 0.0);
#endif    
 }