...
vec3 textureRGB(const in sampler2D texture_in, const in vec2 uv) 
{
    return texture(texture_in, uv.xy ).rgb;
}

vec4 textureRGBA(const in sampler2D texture_in, const in vec2 uv) 
{
    return texture(texture_in, uv.xy ).rgba;
}

float textureIntensity(const in sampler2D texture_in, const in vec2 uv) 
{
    return texture(texture_in, uv).r;
}

float textureAlpha(const in sampler2D texture_in, const in vec2 uv) 
{
    return texture(texture_in, uv.xy ).a;
}
...
{
	vec4 rgba = texture(tex, vec2(NoV, r));
...
vec3 computeIBLDiffuseUE4( const in vec3 normal, const in vec3 albedo, const in mat3 envTrans, const in vec3 sphHarm[9] ) 
{
    return albedo * evaluateDiffuseSphericalHarmonics(sphHarm, envTrans, normal) ;
		...
		vec3 texturePanorama(const in sampler2D texture_in, const in vec2 uv)
		{
			vec4 rgba = texture(texture_in, uv);
		#ifdef FLOAT
			return rgba.rgb;
		#endif
		...
		vec3 texturePanoramaLod(const in sampler2D texture_in, const in vec2 size, const in vec3 direction, const in float lodInput, const in float maxLOD)
		{
			float lod = min(maxLOD, lodInput);
			...
			vec3 texel0 = texturePanorama(texture_in, uv0.xy);
			float lod1 = ceil(lod);
			vec2 uv1 = computeUVForMipmap(lod1, uvBase, size.x, maxLOD);
			vec3 texel1 = texturePanorama(texture_in, uv1.xy);
...
void main() 
{
	...
	#ifdef NORMALMAP
		vec3 texNormalRGB = texture(normalMap, fragmentTexCoord0.xy).rgb;
...
#elif defined ALBEDO
	#ifdef ALBEDOMAP
		vec4 albedoTextureColorAndAlpha = texture(albedoMap, fragmentTexCoord0.xy).rgba;
	#else
		vec4 albedoTextureColorAndAlpha = vec4(1, 1 ,1 ,1);
	#endif
...
#ifdef TRANSPARENCY
	#ifdef TRANSPARENCYMAP
		float textureOpaque = dot(texture(transparencyMap, fragmentTexCoord0.xy).rgba, transparencyChannel);
...
#ifdef AO
	#ifdef AOMAP
		float diffuseAO = dot(texture(aoMap, fragmentTexCoord0.xy).rgba, aoChannel);
	#else
		float diffuseAO = 1.0;
	#endif
...
#ifdef ROUGHNESS
	#ifdef ROUGHNESSMAP
		float roughness = dot(texture(roughnessMap, fragmentTexCoord0.xy).rgba, roughnessChannel);
	#else
		float roughness = 1.0;
	#endif
	//roughness = adjustRoughnessNormalMap(roughness, texNormal);
	roughness = adjustRoughnessGeometry(max(1.e-4, roughness * roughnessFactor), frontNormal);
	...
	vec3 specularLighting = vec3(0, 0, 0);
		...
		#ifdef SPECULAR
			#ifdef SPECULARMAP
				vec3 specularTextureColor = texture(specularMap, fragmentTexCoord0.xy).rgb;
			#else
		...
		#ifdef METALNESS
			#ifdef METALNESSMAP
				float metalness = dot(texture(metalnessMap, fragmentTexCoord0.xy).rgba, metalnessChannel);
			#else
				float metalness = 1.0;
			#endif
		...
		#ifdef SPECULARF0
			#ifdef SPECULARF0MAP
				float specularF0 = dot(texture(specularF0Map, fragmentTexCoord0.xy).rgba, specularF0Channel);
			#else
...
#ifdef EMISSIVE
	#ifdef EMISSIVEMAP
		vec3 emissiveTextureColor = sRGBToLinear(texture(emissiveMap, fragmentTexCoord0.xy).rgb);
	#else
...
#ifdef CAVITY
	#ifdef CAVITYMAP
		float cavityValue = dot(texture(cavityMap, fragmentTexCoord0.xy).rgba, cavityChannel);
	#else
