...
// changelog:  gl_FragColor may be unsupported, change to 'out vec4 FragColor'
//             GL_EXT_shader_texture_lod may be unsupported, change to GL_ARB_shader_texture_lod
...
in vec3 fragmentVextexWorldPosition;
...
#ifdef TRANSPARENCY
    FragColor = vec4(srgbResult * opaqueValue, opaqueValue); //ALPHA BLEND MODE S.rgb + D.rgb * (1.0 - S.a)
#else
    FragColor = encodeRGBM(srgbResult, 0.0); //7e+0 for HDR
#endif
}