...
uniform mat4 viewMatrix;
	...
	uniform int boneTextureSize;
		...
		float decodeRGBA2Float(float dx, float X, float Y, float index)
		{
			...
			vec4 rgba0 = texture(boneTexture, vec2(dx * (X + i0), Y));
			vec4 rgba1 = texture(boneTexture, vec2(dx * (X + i1), Y));
...
out vec3 fragmentVextexViewPosition;
out vec3 fragmentVextexWorldPosition;
