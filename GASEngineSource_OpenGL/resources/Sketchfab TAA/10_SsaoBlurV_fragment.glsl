...
#endif
#define SSAO_TAP_BLUR(id, absid) sampleTex = texture2D(texSsao, uv + axis * float(id) * 2.0);z = dot(sampleTex.gba, vec3( 1.0, 1.0 / 255.0, 1.0 / 65025.0));weight = max(0.0, 1.0 - sharpnessFactor * abs(z - initialZ)) * (0.3 + gaussian[absid]);sum += weight * sampleTex.r;totalWeight += weight;
...
vec3 tmp_0; vec4 tmp_4; vec3 tmp_5; float tmp_6 = 7e+0; vec4 tmp_7; float tmp_8 = 1.0; vec3 tmp_11; float tmp_13 = 7e+0;

// end vars

tmp_4 = textureRGBA( Texture0, vTexCoord0.xy );

tmp_5 = decodeRGBM( tmp_4, tmp_6 );

tmp_7 = vec4( tmp_5.rgb, tmp_8 );
tmp_0 = ssaoBlurV( TextureSsao, vTexCoord0, RenderSize, uNearFar, tmp_7.rgb );
...
gl_FragColor = encodeRGBM( tmp_11, tmp_13 );
