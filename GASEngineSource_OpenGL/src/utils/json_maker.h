...
class JsonDoc {
 ...
 public:
    ...
    template <typename GenericValue, typename AnyType>
    rapidjson::Value& AddMemberTo(GenericValue& v, const std::string& name,
                                  const std::vector<AnyType>& value) {
        assert(v.IsObject());
    ...
    std::enable_if_t<!is_template_of<std::vector, std::decay_t<JsonOrOtherType>>::value,
                     rapidjson::Value&>
