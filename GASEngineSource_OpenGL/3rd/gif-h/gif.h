...
void GifGetClosestPaletteColor(GifPalette* pPal, int r, int g, int b, int* bestInd, int* bestDiff,
                               int treeRoot) {
...
void GifSplitPalette(uint8_t* image, int numPixels, int firstElt, int lastElt, int splitElt,
    ...
    // and split along that axis. (incidentally, this means this isn't a "proper" k-d tree but I
...
void GifThresholdImage(const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame,
    ...
    for (uint32_t ii = 0; ii < numPixels; ++ii) {
        ...
        if (lastFrame && lastFrame[0] == nextFrame[0] && lastFrame[1] == nextFrame[1] &&
            ...
            int32_t bestDiff = 1000000;
            int32_t bestInd = 1;
            GifGetClosestPaletteColor(pPal, nextFrame[0], nextFrame[1], nextFrame[2], &bestInd,
                                      &bestDiff, 1);
...
typedef struct {
    ...
    uint32_t chunkIndex;
    uint8_t chunk[256];  // bytes are written in here until we have 256 of them, then written to the
                         // file
} GifBitStatus;
