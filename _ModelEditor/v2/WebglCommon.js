    ...
    for ( var name in gizmoMap ) {
        ...
        for ( var i = 0; i < gizmoLength; i++ ) 
        {
            ...
            if (translation) 
            {
                ...
                this.tmpVec.set(translation[0], translation[1], translation[2]);
                childEntity.setLocalTranslation(this.tmpVec);
            }
            if (rotation) 
            {
                // childEntity.rotation.set(rotation[ 0 ], rotation[ 1 ], rotation[ 2 ], 'XYZ');
                this.tmpRotation.set(rotation[ 0 ], rotation[ 1 ], rotation[ 2 ], 'XYZ');
                childEntity.setLocalRotation(this.tmpRotation);
            }
            if (scale) 
            {
                // childEntity.scale.set(scale[ 0 ], scale[ 1 ], scale[ 2 ]);
                this.tmpVec.set(scale[0], scale[1], scale[2]);
                childEntity.setLocalScale(this.tmpVec);
            }
