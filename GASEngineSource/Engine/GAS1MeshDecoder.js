...
GASEngine.GAS1MeshDecoder = function()
{
    this._loadFunctions = new Map();

    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_POSITION, 
        function
        (
        ...
        )
        {
            ...
            if(dataAttribute === GASEngine.GAS1MeshDecoder.MESH_ATTRIBUTE_LOOSE)
            {
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_NORMAL0, 
        function
        (
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_TANGENT0, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_UV0, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount, bboxMin, bboxMax
        )
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_VERTEXCOLOR0, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_BLENDWEIGHT, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_BLENDINDEX, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
        {
            ...
            if(dataAttribute === GASEngine.GAS1MeshDecoder.MESH_ATTRIBUTE_LOOSE)
            {
                blendIndices = new Uint16Array(elementCount * attributeCount);
                var output = new Float32Array(dataView.buffer, sectionOffset + 16, elementCount * attributeCount);
                for(var i = 0; i < blendIndices.length; ++i)
                {
                    blendIndices[i] = output[i];
                }
            }
            else
            {
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_INDEX, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_SUBMESH, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_BONE, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
        {
            ...
            for(var j = 0; j < attributeCount; ++j)
            {
                ...
                if(this._objectStringTable_)
                {
                    var nameIndex = dataView.getInt32(offset + 4, true);
                    var name1Index = dataView.getInt32(offset + 8, true);

                    bone.name = this._objectStringTable_[nameIndex];
                    bone.name1 = this._objectStringTable_[name1Index];
                }

                offset += (elementCount - 16)*4;
                bone.matrixWorld2Bone = [];
                for(var matrixElementIndex = 0; matrixElementIndex < 16; ++matrixElementIndex)
                {                            
                    var matrixElementValue = dataView.getFloat32(offset, true);
                    bone.matrixWorld2Bone.push(matrixElementValue);
                    offset += 4;
                }
                boneList.push(bone);
            }
    ...
    this._loadFunctions.set
    (
        GASEngine.GAS1MeshDecoder.MESH_DATA_TOPOLOGY, 
        function
        (
            dataView, sectionOffset, sectionLength, dataAttribute, elementCount, attributeCount
        )
    ...
    this._objectStringTable_ = null;
};

GASEngine.GAS1MeshDecoder.MESH_DATA_POSITION = 0;
GASEngine.GAS1MeshDecoder.MESH_DATA_NORMAL0 = 1;
GASEngine.GAS1MeshDecoder.MESH_DATA_NORMAL1 = 2;
GASEngine.GAS1MeshDecoder.MESH_DATA_TANGENT0 = 3;
GASEngine.GAS1MeshDecoder.MESH_DATA_TANGENT1 = 4;
GASEngine.GAS1MeshDecoder.MESH_DATA_BINORMAL0 = 5;
GASEngine.GAS1MeshDecoder.MESH_DATA_BINORMAL1 = 6;
GASEngine.GAS1MeshDecoder.MESH_DATA_UV0 = 7;
GASEngine.GAS1MeshDecoder.MESH_DATA_UV1 = 8;
GASEngine.GAS1MeshDecoder.MESH_DATA_VERTEXCOLOR0 = 9;
GASEngine.GAS1MeshDecoder.MESH_DATA_VERTEXCOLOR1 = 10;
GASEngine.GAS1MeshDecoder.MESH_DATA_BLENDWEIGHT = 11;
GASEngine.GAS1MeshDecoder.MESH_DATA_BLENDINDEX = 12;
GASEngine.GAS1MeshDecoder.MESH_DATA_INDEX = 13;
GASEngine.GAS1MeshDecoder.MESH_DATA_SUBMESH = 14;
GASEngine.GAS1MeshDecoder.MESH_DATA_BONE = 15;
GASEngine.GAS1MeshDecoder.MESH_DATA_MORPHTARGET = 16;
GASEngine.GAS1MeshDecoder.MESH_DATA_KEYFRAME = 17;
GASEngine.GAS1MeshDecoder.MESH_DATA_TOPOLOGY = 18;
GASEngine.GAS1MeshDecoder.MESH_DATA_TOTAL_COUNT = 19;
GASEngine.GAS1MeshDecoder.MESH_DATA_UNKNOWN = 255;

GASEngine.GAS1MeshDecoder.MESH_ATTRIBUTE_LOOSE = 0;
GASEngine.GAS1MeshDecoder.MESH_ATTRIBUTE_COMPRESSED = 1;

GASEngine.GAS1MeshDecoder.SHADER_STREAM_ATTRIBUTE_NAMES = [
        'position',
        'normal',
        'normal1',
        'tangent',
...
GASEngine.GAS1MeshDecoder.NORMAL_DECODING_DICTIONARY = [
...
GASEngine.GAS1MeshDecoder.NORMAL_ENCODING_PHI_COUNT = 120;

GASEngine.GAS1MeshDecoder.prototype = {

    constructor: GASEngine.GAS1MeshDecoder,
    ...
    _decodeNormals: function(dataView, offset, sectionLength, encodingFlag, elementCount, attributeCount)
    {
        ...
        var k = Math.PI / (GASEngine.GAS1MeshDecoder.NORMAL_ENCODING_PHI_COUNT - 1);
        for(var i = 0; i < attributeCount; ++i)
        {
            ...
            var theta = (2.0 * encodedTheta * Math.PI) / GASEngine.GAS1MeshDecoder.NORMAL_DECODING_DICTIONARY[encodedPhi];
    ...
    _constructStringTable: function(dataView, offset)
    {
        var entryCount = dataView.getInt32(offset, true);
        offset += 4;

        this._objectStringTable_ = [];
        for(var i = 0; i < entryCount; ++i)
        {
            ...
            this._objectStringTable_.push(jsString);
        }
    },

    _parseMesh: function(dataView, beginningOffset)
    {
        ...
        if(version == 7)
        {
            objectSize = dataView.getUint32(beginningOffset + 4, true);
        }
        else
        {
            objectSize = dataView.byteLength;
        }
        ...
        var objectNameIndex = dataView.getUint32(beginningOffset + 40, true)
        ...
        var stringTableOffset = dataView.getInt32(beginningOffset + 84, true);
        if(stringTableOffset > 0)
        {
            this._constructStringTable(dataView, beginningOffset + stringTableOffset);
        }

        var meshName = '';
        if(this._objectStringTable_ !== null)
        {
            meshName = this._objectStringTable_[objectNameIndex];
        }        
        mesh.name = meshName;
        ...
        for(var i = 0; i < sectionCount; ++i)
        {
            ...
            if(sectionType === GASEngine.GAS1MeshDecoder.MESH_DATA_POSITION)
            {
                ...
                if(stream !== null)
                {
                    mesh.addStream(GASEngine.GAS1MeshDecoder.SHADER_STREAM_ATTRIBUTE_NAMES[sectionType], stream);
                }
            }
            else if(sectionType === GASEngine.GAS1MeshDecoder.MESH_DATA_UV0 ||
                sectionType === GASEngine.GAS1MeshDecoder.MESH_DATA_UV1)
            {
                var stream = this._loadFunctions.get(GASEngine.GAS1MeshDecoder.MESH_DATA_UV0)(
                    dataView,
                    sectionOffset,
                    sectionLength,
                    dataAttribute,
        ...
        if(normalStream === null)
        {
            ...
            {
                console.error('GASEngine.GAS1MeshDecoder.parseMesh: unable to compute vertex normals for inadequate source stream!');
            }
        }
        ...
        var uvtopologyStream = mesh.getStream('uvtopology');
        if (uvtopologyStream === null)
        {
            var indices = mesh.getStream('index');
            if (indices !== null)
                mesh.addStream('uvtopology', mesh.createUVTopologicalIndices(indices));
        }
    ...
    parse: function(dataView)
    {
        ...
        {
            var parentUniqueID = dataView.getInt32(offset + 20, true);
            var meshUniqueID = dataView.getInt32(offset + 24, true);

            var result = this._parseMesh(dataView, offset);
