GASEngine.FirstPersonManipulatorStandardMouseKeyboardController = function(manipulator)
{
    this._manipulator = manipulator;
    this.init();
};

GASEngine.FirstPersonManipulatorStandardMouseKeyboardController.prototype =
{
    init: function()
    {
        this.releaseButton();
    ...
    {
        this.releaseButton();
        this._manipulator.getForwardInterpolator().reset();
        this._manipulator.getSideInterpolator().reset();
    },
    ...
    keydown: function(event)
    {
        ...
        else if(event.keyCode === 65 || event.keyCode === 37)// a/q/left
        {
            manipulator.getSideInterpolator().setDelay(this._delay);
            manipulator.getSideInterpolator().setTarget(-1);
            event.preventDefault();
    ...
    {
        var manipulator = this._manipulator;
        if(event.keyCode === 87 || event.keyCode === 38 || // w/z/up
        ...
        { 
            ...
            event.keyCode === 65 || event.keyCode === 37)// a/q/left
        { 
            manipulator.getSideInterpolator().setDelay(this._delay);
