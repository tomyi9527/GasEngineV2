...
uniform sampler2D sTextureDiffusePBR;
uniform sampler2D sTextureRoughnessPBR;
uniform sampler2D sTextureSpecularPBR;
...
// vars
	...
	mat3 tmp_0; vec3 tmp_3; vec3 geoNormal; vec3 normal; vec3 frontNormal; vec3 materialDiffusePBR; vec3 materialAlbedo; vec3 tmp_12; vec3 tmp_13; vec3 channelDiffusePBR; vec3 tmp_16; const float floatWhite = float(1.0); vec3 tmp_19; vec3 tmp_20; vec3 eyeVector; float tmp_23 = -1.0; float materialRoughness; float tmp_26; float channelRoughnessPBR; float tmp_29; vec3 tmp_31; vec3 tmp_32; vec3 channelSpecularPBR; vec3 tmp_38; vec3 tmp_39; float materialAO = 1.0; vec3 tmp_41; vec3 tmp_42; vec3 tmp_44; vec3 tmp_45;
	...
	tmp_12 = textureRGB( sTextureDiffusePBR, vTexCoord0.xy );

	tmp_13 = sRGBToLinear( tmp_12 );

	channelDiffusePBR = tmp_13.rgb*uDiffusePBRFactor;
	...
	tmp_16 = tmp_3.rgb*floatWhite;
	tmp_20 = vViewVertex.rgb;
	tmp_19 = normalize( tmp_20 );

	eyeVector = tmp_19.rgb*tmp_23;
	tmp_26 = textureIntensity( sTextureRoughnessPBR, vTexCoord0.xy );

	channelRoughnessPBR = tmp_26*uRoughnessPBRFactor;
	tmp_29 = max(1.e-4, channelRoughnessPBR);
	materialRoughness = adjustRoughnessGeometry( tmp_29, normal );

	tmp_31 = textureRGB( sTextureSpecularPBR, vTexCoord0.xy );

	tmp_32 = sRGBToLinear( tmp_31 );

	channelSpecularPBR = tmp_32.rgb*uSpecularPBRFactor;
	tmp_38 = computeIBLSpecularUE4( geoNormal, eyeVector, materialRoughness, channelSpecularPBR, tmp_0, sSpecularPBR, uTextureEnvironmentSpecularPBRLodRange, uTextureEnvironmentSpecularPBRTextureSize, normal, sIntegrateBRDF );

	tmp_39 = tmp_38.rgb*materialAO;
	tmp_41 = tmp_16.rgb+tmp_39.rgb;
	tmp_42 = tmp_41.rgb*uEnvironmentExposure;
	tmp_44 = tmp_42.rgb*floatWhite;
	if(uOutputLinear == 1 ) {
	tmp_45= tmp_44;
	} else {
	tmp_45 = linearTosRGB( tmp_44 );
	}
	gl_FragColor = encodeRGBM( tmp_45, uRGBMRange );
