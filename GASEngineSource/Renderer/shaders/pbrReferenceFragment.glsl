...
vec3 getReferenceTexelEnvironmentLod( const in vec3 dirLocal, const in float pdf ) {
    ...
    float textureSize = float(uEnvironmentSpecularTextureSize[0]);
    ...
    return textureCubemapLod( environmentSpecularCube, direction, lod ).rgb;
#else
...
void main(void) {
	...
	//vec3 viewLight = (viewMatrix*vec4(1, 0, 0, 0)).xyz;
	//
	//float k = dot(viewLight, normal);
    ...
    vec3 albedo = albedoFactor * sRGBToLinear( albedoSource.rgb, DefaultGamma );
...
#endif
    vec4 result = vec4( resultIBL*uEnvironmentExposure, albedoSource.a);
