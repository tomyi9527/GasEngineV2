...
public:
	...
	struct PmxVertex
	{
		...
		void ReadVertex(BinReader & reader, PmxReader * owner)
		{
			...
			case DeformType::QDEF://Dual quaternion deform blending
				Weight[0].Bone = ReadElement_Int32(reader, owner->BoneSize, true);
				Weight[1].Bone = ReadElement_Int32(reader, owner->BoneSize, true);
			...
			case DeformType::SDEF://Spherical deform blending
				Weight[0].Bone = ReadElement_Int32(reader, owner->BoneSize, true);
				Weight[1].Bone = ReadElement_Int32(reader, owner->BoneSize, true);
				Weight[0].Value = reader.Read<float>();
				Weight[1].Value = 1 - Weight[0].Value;
	...
	struct MaterialFlags
	{
		enum Type
		{
			None = 0,
			DrawBoth = 0x01,
			Shadow = 0x02,			//draw ground shadow
			SelfShadowMap = 0x04, //draw shadow
			SelfShadow = 0x08,//recevie shadow
			Edge = 0x10,//Has "pencil" outline
			VertexColor = 0x20,
			PointDraw = 0x40,			
			LineDraw = 0x80
		};
	...
	struct PmxMaterial
	{
		...
		void ReadMaterial(BinReader & reader, PmxReader * owner)
		{
			...
			Sphere = owner->IndexToName[ReadElement_Int32(reader, owner->TexSize, true)]; //environment mapping
			SphereMode = (SphereModeType::Type)reader.Read<uint8_t>(); //Environment blend mode
	...
	struct BoneFlags
	{
		...
		{
			None = 0x0,
			ToBone = 0x01, //Is the tail position a vec3 or bone index
			Rotation = 0x02, //Enables rotation
			Translation = 0x04, //Enables translation (shear)
			Visible = 0x08,
			Enable = 0x10,
			IK = 0x20, //Use inverse kinematics (physics)
			AffectLocal = 0x80, //Link constraint param
			AffectRotation = 0x100, //Rotation inherits from another bone
			AffectTranslation = 0x200, //Translation inherits from another bone
			FixAxis = 0x400, //The bone's shaft is fixed in a direction
			LocalFrame = 0x800,
			AfterPhysics = 0x1000,
			ExtParent = 0x2000
		};
	...
	struct PmxIK
	{
		...
		{
			Effector = ReadElement_Int32(reader, owner->BoneSize, true);
			Iteration = ReadElement_Int32(reader, 4, true);
			...
			for (int i = 0; i < num; ++i)
			{
				LinkList[i].ReadLink(reader, owner);
			}
	...
	struct PmxBone
	{
		...
		PmxBone()
		{
			...
			LinkConstraintParent = -1;
			LinkConstraintRatio = 1.0f;
			Priority = 0;
			ExtKey = 0;
		...
		void ReadBone(BinReader& reader, PmxReader* owner)
		{
			...
			Priority = ReadElement_Int32(reader, 4, true); //transformationClass
			...
			LinkConstraintAffectTranslation = GetFlag(BoneFlags::AffectTranslation);
			LinkConstraintAffectRotation = GetFlag(BoneFlags::AffectRotation);
			LinkConstraintAffectLocal = GetFlag(BoneFlags::AffectLocal);
			if (LinkConstraintAffectTranslation || LinkConstraintAffectRotation) //link constraint
			{
				LinkConstraintParent = ReadElement_Int32(reader, owner->BoneSize, true);
				LinkConstraintRatio = reader.Read<float>();
			}
	...
	struct PmxNode
	{
		...
		void ReadNode(BinReader& reader, PmxReader* owner)
		{
			...
			{
				ElementList[i].Read(reader, owner);
			}
		}
	};
	...
	struct ModeType
	{
		enum Type
		{
			BoneConnected,//FollowBone
			Physics,//Physics
			ConnectedPhysics//PhysicsAndBone
		};
	};
	...
	{
		std::string			Name, NameE;
		int					Bone, GroupIndex, GroupTarget;
		...
		void ReadRigidBody(BinReader & reader, PmxReader * owner)
		{
			owner->ReadString(Name);
			owner->ReadString(NameE);
			Bone = ReadElement_Int32(reader, owner->BoneSize, true);
			GroupIndex = ReadElement_Int32(reader, 1, true);
	...
	struct PmxSoftBody
	{
		...
		void NormalizeBodyAnchorAndVertexPin()
		{
			...
			//}
			for (int i = 0; i < BodyAnchorList.size(); ++i)
			{
				if (optAnchor.find(BodyAnchorList[i]) == optAnchor.end())
					optAnchor.insert(BodyAnchorList[i]);
			}
			...
			//}
			for (std::set<BodyAnchor>::iterator iter = optAnchor.begin(); iter != optAnchor.end(); ++iter)
			{
				BodyAnchorList.push_back(*iter);
			}
			...
			//		optPin.insert(item);
			//}
			for (int i = 0; i < VertexPinList.size(); ++i)
			{
				if (optPin.find(VertexPinList[i]) == optPin.end())
					optPin.insert(VertexPinList[i]);
			}
			...
			//}
			for (std::set<VertexPin>::iterator iter = optPin.begin(); iter != optPin.end(); ++iter)
			{
				VertexPinList.push_back(*iter);
			}
		}
	};
...
private:
	void ReadHeader()
	{
		...
		else
		{
			if (strMagic != PmxKey)
			{
				throw std::string("PMX file format error!");
			}
		...
		if (Ver > 2.1f)
		{
			throw std::string("Do not support the version bigger than 2.1!");
		}
	...
	void ReadVertexInfo()
	{
		...
		for (int i = 0; i < num; ++i)
		{
			VertexList[i].ReadVertex(m_Reader, this);
		}
	}
	...
	void ReadMaterialInfo()
	{
		...
		for (int i = 0; i < num; ++i)
		{
			MaterialList[i].ReadMaterial(m_Reader, this);
		}
	}
	...
	void ReadBoneInfo()
	{
		...
		for (int i = 0; i < num; ++i)
		{
			BoneList[i].ReadBone(m_Reader, this);
		}
		...
		for (int i = 0; i < num; ++i)
		{
			...
			{
				BoneList[i].RefParent = &BoneList[BoneList[i].Parent];
			}
		}
	...
	void ReadMorphInfo()
	{
		...
		for (int i = 0; i < num; ++i)
		{
			MorphList[i].ReadMorph(m_Reader, this);
		}
	}
	...
	void ReadRigidBodyInfo()
	{
		...
		//{
		//	item.ReadRigidBody(m_Reader, this);
		//}
		for (int i = 0; i < num; ++i)
		{
			BodyList[i].ReadRigidBody(m_Reader, this);
		}
	}
	...
	void ReadJointInfo()
	{
		...
		for (int i = 0; i < num; ++i)
		{
			JointList[i].ReadJoint(m_Reader, this);
		}
	}
	...
	void ReadSoftBodyInfo()
	{
		if (Ver >= 2.1f)
		{
			...
			for (int i = 0; i < num; ++i)
			{
				SoftBodyList[i].ReadSoftBody(m_Reader, this);
			}
		}
	}
...
public:
	...
	std::vector<PmxRigidBody>				BodyList;
