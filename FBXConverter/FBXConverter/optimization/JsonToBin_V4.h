...
class JsonToBin_V4 : public FileWriter
{
public:
	JsonToBin_V4();
	virtual ~JsonToBin_V4();
...
private:
	...
	char* composeStringTable_V1(std::vector<string>& strings, unsigned int& stringTableSize);

	void writeObjectBinFile(
		BIN_HEADER* header,
		SECTION_DATA* sections,
		char* stringTable,
		unsigned int stringTableSize);

	void writeAnimationBinFile(
...
private:
	FILE* mBinFile;
	std::map<std::string, int> mStringTable_V1;

	SECTION_DATA* mSections;
};
