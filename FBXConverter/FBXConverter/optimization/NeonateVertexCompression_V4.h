...
class NeonateVertexCompression_V4
{
public:
	NeonateVertexCompression_V4();
	~NeonateVertexCompression_V4();
	...
	FBXSDK_NAMESPACE::FbxVector4* computeVertexNormals
	(
		FBXSDK_NAMESPACE::FbxVector4* position,
		FBXSDK_NAMESPACE::FbxUInt32	positionCount,
		std::vector<int>&			indices
	);
	...
	bool appendNewLayer_V2
	(
		std::vector<std::vector<int>*>& vb,
		std::vector<int>&			newLayerIndices,
		std::vector<int>&			indices,
		VERTEX_LAYER_TYPE			layerType
	);
	...
	void rearrangeTrianglesByMaterial
	(
		const std::vector<std::vector<int> >& subMeshes,
	...
	inline MESH_DETAIL getMeshDetail()
	{
		return mMeshDetail;
	}

public:
	static FbxVector4 transformPositionWithGeometricTransform
	(
		const FBXSDK_NAMESPACE::FbxAMatrix& meshGeometry, 
		FbxDouble x, 
		FbxDouble y, 
		FbxDouble z
	);

	static FbxVector4 transformNormalWithGeometricTransform
	(
		const FBXSDK_NAMESPACE::FbxAMatrix& meshGeometry, FbxDouble x, FbxDouble y, FbxDouble z
	);

	static FbxAMatrix getGeometryMatrix(FBXSDK_NAMESPACE::FbxNode* node);

	static FbxAMatrix getWorldToBoneLocalMatrixAtBindPose
	(
		FBXSDK_NAMESPACE::FbxCluster* cluster, 
		FBXSDK_NAMESPACE::FbxNode* meshNode
	);
