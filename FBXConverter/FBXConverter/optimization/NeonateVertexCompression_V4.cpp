...
NeonateVertexCompression_V4::NeonateVertexCompression_V4()
{
}
...
void NeonateVertexCompression_V4::compressVertices
(
...
)
{
	...
	//Tangents
	bool result0 = mesh->GenerateTangentsData(0, false);

	//
	int polygonCount = mesh->GetPolygonCount();
	polygonCountOfMesh = polygonCount;
	mMeshDetail.polygonCount = polygonCount;
	...
	mMeshDetail.triangleCount = totalTriangleCount;
	...
	for (int i = 0;i < effectiveBones->size();i++)
	{
		mMeshDetail.bones[(*effectiveBones)[i].id] = (*effectiveBones)[i].name;
		mMeshDetail.boneHierarchy[(*effectiveBones)[i].id] = (*effectiveBones)[i].parentID;
	}

	//Position layer
	int positionCount = mesh->GetControlPointsCount();
	vertexCountOfMesh = positionCount;
	mMeshDetail.vertexCount = positionCount;
	...
	//Append UV1
	vertexElementCount = getUVIndices(mesh, nextIndices, 1, triangleCountPerPolygon, triangleIndices);
	if (vertexElementCount > 0)
	{
		appendNewLayer_V2(vertexLayers, nextIndices, indices, VL_UV1);
	}
	...
	std::vector<int>* positionIndices = NeonateVertexCompression_V4::getVertexLayer(mVertexElements, vertexLayers, VL_POSITION);
	std::vector<int>* normalIndices = NeonateVertexCompression_V4::getVertexLayer(mVertexElements, vertexLayers, VL_NORMAL0);
	std::vector<int>* uv0Indices = NeonateVertexCompression_V4::getVertexLayer(mVertexElements, vertexLayers, VL_UV0);
	std::vector<int>* uv1Indices = NeonateVertexCompression_V4::getVertexLayer(mVertexElements, vertexLayers, VL_UV1);
	std::vector<int>* tangentIndices = NeonateVertexCompression_V4::getVertexLayer(mVertexElements, vertexLayers, VL_TANGENT0);
	...
	//mMeshDetail.vertexFormats = mVertexElements;
	...
	//UV0 outputs
	FBXSDK_NAMESPACE::FbxDouble3 uv0BboxMin(FLT_MAX, FLT_MAX, FLT_MAX);
	FBXSDK_NAMESPACE::FbxDouble3 uv0BboxMax(FLT_MIN, FLT_MIN, FLT_MIN);
	uint8_t* outUV0EncodingBuffer = NULL;
	unsigned int outUV0EncodingSize = 0;
	unsigned int outUV0EncodingFlag = 0;
	if (uv0Indices != NULL)
	{
		outUV0EncodingBuffer = new uint8_t[totalVertexCount*sizeof(float) * 2];
	}

	//UV1 outputs
	FBXSDK_NAMESPACE::FbxDouble3 uv1BboxMin(FLT_MAX, FLT_MAX, FLT_MAX);
	FBXSDK_NAMESPACE::FbxDouble3 uv1BboxMax(FLT_MIN, FLT_MIN, FLT_MIN);
	uint8_t* outUV1EncodingBuffer = NULL;
	unsigned int outUV1EncodingSize = 0;
	unsigned int outUV1EncodingFlag = 0;
	if (uv1Indices != NULL)
	{
		outUV1EncodingBuffer = new uint8_t[totalVertexCount * sizeof(float) * 2];
	}
	...
	std::vector<int>* vertexColorIndices = NeonateVertexCompression_V4::getVertexLayer(mVertexElements, vertexLayers, VL_VERTEXCOLOR0);
	...
	(
		...
		uv0BboxMin,
		uv0BboxMax,
		outUV0EncodingBuffer,
		outUV0EncodingSize,
		outUV0EncodingFlag,
		\
		uv1BboxMin,
		uv1BboxMax,
		outUV1EncodingBuffer,
		outUV1EncodingSize,
		outUV1EncodingFlag,
		\
		outBWEncodingBuffer,
	...
	unsigned int parentNodeID = (unsigned int)node->GetUniqueID();
	unsigned int nodeID = (unsigned int)mesh->GetUniqueID();

	float posBoundingBoxMin[3] = { (float)bboxMin[0], (float)bboxMin[1], (float)bboxMin[2] };
	float posBoundingBoxMax[3] = { (float)bboxMax[0], (float)bboxMax[1], (float)bboxMax[2] };
...
std::vector<int>* NeonateVertexCompression_V4::getVertexLayer
(
...
void NeonateVertexCompression_V4::optimizationAndWriteMorphTarget
(
	FileWriter*			writer,
	unsigned int		parentMeshUniqueID,
...
)
{
	...
	for (int i = 0; i < morphTargetCount; ++i)
	{
		...
		//Encode positions
		size_t posBufferSize = data.positions.size()*sizeof(float) + sizeof(unsigned int) * 6;
		uint8_t* outPosEncodingBuffer = new uint8_t[posBufferSize];
		unsigned int outPosEncodingSize = 0;
		unsigned int outPosEncodingFlag = 0;
		...
		{
			outPosEncodingSize = positionEncoding(data.positions, outPosEncodingFlag, data.bboxMin, data.bboxMax, outPosEncodingBuffer);
			outPosEncodingSize = roundUp(outPosEncodingSize, 4);
		}
		else
		{
			outPosEncodingSize = (unsigned int)(data.positions.size()*sizeof(float));
			memcpy(outPosEncodingBuffer, &data.positions[0], outPosEncodingSize);
		...
		if (data.normals.size() > 0 && data.normals.size() == data.positions.size())
		{
			...
			{
				outNormalEncodingSize = encodeNormals(vertexCount, data.normals, outNormalEncodingBuffer);
			...
			else
			{
				...
				memcpy(outNormalEncodingBuffer, &data.normals[0], outNormalEncodingSize);
				outNormalEncodingFlag = 0;
			}
		}

		float posBoundingBoxMin[3] = { (float)data.bboxMin[0], (float)data.bboxMin[1], (float)data.bboxMin[2] };
		float posBoundingBoxMax[3] = { (float)data.bboxMax[0], (float)data.bboxMax[1], (float)data.bboxMax[2] };
...
#ifdef _MSC_VER
void outputBuffer(const std::wstring& outputFile, unsigned char* buffer, size_t bufferSize)
{
...
void NeonateVertexCompression_V4::meshOptimization
(
	FbxMesh*						mesh,
	unsigned int					optimizationFlag,
	std::vector<VERTEX_LAYER_TYPE>& vertexElement,
	...
	uint8_t*		outUV0EncodingBuffer,
	unsigned int&	outUV0EncodingSize,
	unsigned int&	outUV0EncodingFlag,
	\
	FBXSDK_NAMESPACE::FbxDouble3& uv1BboxMin,
	FBXSDK_NAMESPACE::FbxDouble3& uv1BboxMax,
	uint8_t*		outUV1EncodingBuffer,
	unsigned int&	outUV1EncodingSize,
	unsigned int&	outUV1EncodingFlag,
	\
...
)
{
	...
	//outputBuffer(L"d:\\positions.bin", (unsigned char*)(&outputReorderedPositions[0]), outputReorderedPositions.size()* sizeof(float));
	//Encoding positions
	if (optimizationFlag & (1 << VL_POSITION))
	{
	...
	mMeshDetail.vertexFormats.push_back(VL_POSITION);

	//Encoding Indices
	if (optimizationFlag & (1 << VL_INDEX))
	{
		NeonateVertexCompression_V4::deltaAndZigzagEncoding(outIndices);
		outIndexEncodingSize = NeonateVertexCompression_V4::varintEncoding(outIndices, outIndexEncodingBuffer);
	...
	if (optimizationFlag & (1 << VL_TOPOLOGICALINDEX))
	{
		...
		NeonateVertexCompression_V4::deltaAndZigzagEncoding(topologicalIndices);
		outTopologicalIndexEncodingSize = NeonateVertexCompression_V4::varintEncoding(topologicalIndices, outTopologicalIndexEncodingBuffer);
	...
	//Encoding UV0s
	std::vector<int>* uv0Indices = NeonateVertexCompression_V4::getVertexLayer(vertexElement, vertexLayers, VL_UV0);
	std::vector<float> orginalUV0Data;
	if (uv0Indices != NULL)
	{		
		getUVs(mesh, uv0Indices, 0, orginalUV0Data, uv0BboxMin, uv0BboxMax);

		//outputBuffer(L"d:\\uv0s.bin", (unsigned char*)(&orginalUV0Data[0]), orginalUV0Data.size() * sizeof(float));

		if (optimizationFlag & (1 << VL_UV0))
		{		
			outUV0EncodingSize = encodeUVs(vertexCount, orginalUV0Data, uv0BboxMin, uv0BboxMax, outUV0EncodingBuffer);
			outUV0EncodingSize = roundUp(outUV0EncodingSize, 4);
			outUV0EncodingFlag = 1;
		}
		else
		{
			outUV0EncodingSize = (unsigned int)(vertexCount*sizeof(float) * 2);
			memcpy(outUV0EncodingBuffer, &orginalUV0Data[0], outUV0EncodingSize);
			outUV0EncodingFlag = 0;
		}

		mMeshDetail.vertexFormats.push_back(VL_UV0);
	}

	//Encoding UV1s
	std::vector<int>* uv1Indices = NeonateVertexCompression_V4::getVertexLayer(vertexElement, vertexLayers, VL_UV1);
	std::vector<float> orginalUV1Data;
	if (uv1Indices != NULL)
	{
		getUVs(mesh, uv1Indices, 1, orginalUV1Data, uv1BboxMin, uv1BboxMax);

		//outputBuffer(L"d:\\uv1s.bin", (unsigned char*)(&orginalUV1Data[0]), orginalUV1Data.size() * sizeof(float));

		if (optimizationFlag & (1 << VL_UV1))
		{
			outUV1EncodingSize = encodeUVs(vertexCount, orginalUV1Data, uv1BboxMin, uv1BboxMax, outUV1EncodingBuffer);
			outUV1EncodingSize = roundUp(outUV1EncodingSize, 4);
			outUV1EncodingFlag = 1;
		}
		else
		{
			outUV1EncodingSize = (unsigned int)(vertexCount * sizeof(float) * 2);
			memcpy(outUV1EncodingBuffer, &orginalUV1Data[0], outUV1EncodingSize);
			outUV1EncodingFlag = 0;
		}
		mMeshDetail.vertexFormats.push_back(VL_UV1);
	}

	//Encoding Normals
	std::vector<int>* normalDataIndices = NeonateVertexCompression_V4::getVertexLayer(vertexElement, vertexLayers, VL_NORMAL0);
	...
	if (normalDataIndices != NULL)
	{
		...
		mMeshDetail.vertexFormats.push_back(VL_NORMAL0);
	}

	////Encoding Tangents
	//std::vector<int>* tangentDataIndices = NeonateVertexCompression_V4::getVertexLayer(vertexElement, vertexLayers, VL_TANGENT0);
	...
	//Encoding Tangents
	std::vector<int>* tangentDataIndices = NeonateVertexCompression_V4::getVertexLayer(vertexElement, vertexLayers, VL_TANGENT0);
	//std::vector<int>* binormalDataIndices = NeonateVertexCompression_V4::getVertexLayer(vertexElement, vertexLayers, VL_BINORMAL0);
	if (tangentDataIndices != NULL)
	{
		...
		mMeshDetail.vertexFormats.push_back(VL_TANGENT0);
	}

	//Encoding Vertex Colors
	std::vector<int>* vertexColorDataIndices = NeonateVertexCompression_V4::getVertexLayer(vertexElement, vertexLayers, VL_VERTEXCOLOR0);
	if (vertexColorDataIndices != NULL)
	{
		...
		mMeshDetail.vertexFormats.push_back(VL_VERTEXCOLOR0);
	}
	...
	if (bwValues->size() > 0 && biValues->size() > 0)
	{
		...
		else
		{
			...
			std::vector<float> orginalBIData;
			...
			outBIEncodingSize = (unsigned int)(vertexCount*sizeof(float) * 4);
			memcpy(outBWEncodingBuffer, &orginalBWData[0], outBWEncodingSize);
			memcpy(outBIEncodingBuffer, &orginalBIData[0], outBIEncodingSize);
			outBWEncodingFlag = 0;
			outBIEncodingFlag = 0;
		}
		mMeshDetail.vertexFormats.push_back(VL_BLENDWEIGHT);
		mMeshDetail.vertexFormats.push_back(VL_BLENDINDEX);
		mMeshDetail.vertexFormats.push_back(VL_INDEX);
		mMeshDetail.vertexFormats.push_back(VL_TOPOLOGICALINDEX);
	}
}

unsigned int NeonateVertexCompression_V4::encodeTangents
(
	unsigned int				vertexCount,
...
)
{
	...
	for (unsigned int i = 0; i < vertexCount; ++i)
	{
		...
		float z = input[i * 4 + 2];
		float w = input[i * 4 + 3];
		NeonateVertexCompression_V4::mapNormal(x, y, z, (w - 1.0) / 2.0, encodedPhi, encodeTheta);
...
unsigned int NeonateVertexCompression_V4::encodeNormals
(
	unsigned int				vertexCount,
	const std::vector<float>&	input,
	uint8_t*					outBuffer
...
{	
	...
	for (unsigned int i = 0; i < vertexCount; ++i)
	{
		...
		float y = input[i * 3 + 1];
		float z = input[i * 3 + 2];
		NeonateVertexCompression_V4::mapNormal(x, y, z, 0, encodedPhi, encodeTheta);
	...
	//	//NeonateVertexCompression_V4::mapNormal(v[0], v[1], v[2], encodedPhi, encodeTheta);
	//	FBXSDK_NAMESPACE::FbxVector4 encodedSubFrameVector = NeonateVertexCompression_V4::mapNormal(mappedX, mappedY, mappedZ, encodedPhi, encodeTheta);
...
unsigned int NeonateVertexCompression_V4::encodeUVs
(
	unsigned int						vertexCount,
	const std::vector<float>&			input,
...
unsigned int NeonateVertexCompression_V4::encodeVertexColors
(
	unsigned int	vertexCount,
...
void NeonateVertexCompression_V4::encodeBlendWeightsAndIndices
(
...
)
{
	...
	outBWEncodingSize = NeonateVertexCompression_V4::varintEncoding(blendWeights, outBWEncodingBuffer);
	...
	outBIEncodingSize = NeonateVertexCompression_V4::varintEncoding(blendIndices, outBIEncodingBuffer);
}

void NeonateVertexCompression_V4::rearrangeTrianglesByMaterial(const std::vector<std::vector<int> >& subMeshes, const std::vector<int>& indices, \
...
bool NeonateVertexCompression_V4::ShuffleVertexIndexOrder
(
	std::vector<std::vector<int>*>& vertexLayers, 
...
bool NeonateVertexCompression_V4::ShuffleTopologicalIndexOrder
(
	std::vector<unsigned int>& topologicalIndices, 
	std::vector<unsigned int>& vertexRemapping
)
...
void NeonateVertexCompression_V4::getPositions
(
	FbxMesh*						mesh,
	std::vector<int>*				dataIndices,
	bool							isSkinned, 
...
)
{
	...
	{
		geometricMatrix = NeonateVertexCompression_V4::getGeometryMatrix(node);
	}
	...
	if (!identicalMatrix)
	{
		for (int i = 0; i < positionCount; ++i)
		{
			...
			temp[i] = NeonateVertexCompression_V4::transformPositionWithGeometricTransform(geometricMatrix, p[0], p[1], p[2]);
		}
	}
...
void NeonateVertexCompression_V4::getUVs
(
	FBXSDK_NAMESPACE::FbxMesh*		mesh,
	std::vector<int>*				dataIndices,
	int								index,
...
{
	unsigned int vertexCount = (unsigned int)dataIndices->size();
	FBXSDK_NAMESPACE::FbxLayer* layer = mesh->GetLayer(index, FBXSDK_NAMESPACE::FbxLayerElement::eUV);
...
void NeonateVertexCompression_V4::getNormals
(
	FbxMesh*			mesh,
	std::vector<int>*	dataIndices,
	bool				isSkinned,
...
)
{
	...
	//		normalArray[i] = NeonateVertexCompression_V4::transformNormalWithGeometricTransform(geometricMatrix, n[0], n[1], n[2]);
	//	}
	//}
	...
	for (unsigned int elementIndex = 0; elementIndex < vertexCount; ++elementIndex)
	{
		...
		if (!identicalMatrix)
		{
			v = NeonateVertexCompression_V4::transformNormalWithGeometricTransform(geometricMatrix, v[0], v[1], v[2]);
		}
...
void NeonateVertexCompression_V4::getTangents
(
	FBXSDK_NAMESPACE::FbxMesh*	mesh,
	std::vector<int>*			dataIndices,
	bool						isSkinned,
...
void NeonateVertexCompression_V4::getBinormals
(
	FbxMesh*			mesh,
	std::vector<int>*	dataIndices,
	bool				isSkinned,
...
//void NeonateVertexCompression_V4::getTangents_V1(
//	FbxMesh* mesh,
...
void NeonateVertexCompression_V4::getVertexColors
(
	FbxMesh*				mesh,
	std::vector<int>*		dataIndices,
	bool&					unifiedColor,
...
bool NeonateVertexCompression_V4::getSmoothing
(
...
void NeonateVertexCompression_V4::getBlendWeightsAndIndices
(
	std::vector<int>*	dataIndices,
	std::vector<BW_>*	bwValues,
	std::vector<BI_>*	biValues,
	std::vector<float>& outputBW,
	std::vector<float>& outputBI
)
{
	unsigned int vertexCount = (unsigned int)dataIndices->size();
	...
	for (unsigned int i = 0; i < vertexCount; ++i)
	{
		int k = (*dataIndices)[i];
		BI_& bi = (*biValues)[k];
		outputBI[i * 4 + 0] = (float)bi.data[0];
		outputBI[i * 4 + 1] = (float)bi.data[1];
		outputBI[i * 4 + 2] = (float)bi.data[2];
		outputBI[i * 4 + 3] = (float)bi.data[3];
	}
}

bool NeonateVertexCompression_V4::ReorderTriangles
(
	unsigned int				vertexCount,
...
)
{
	...
	result = TootleOptimizeVertexMemory
	(
		&vertexInputPrefetchOptimization[0], 
		&indicesVtxCacheOptimization[0], 
		vertexCount, 
		meshTriangleCount, 
		vertexStride,
...
void NeonateVertexCompression_V4::highWatermarkEncoding(std::vector<unsigned int>& input)
{
...
void NeonateVertexCompression_V4::highWatermarkDecoding(std::vector<unsigned int>& input)
{
	size_t count = input.size();
	long long highWatermark = 0;
	for (size_t s = 0; s < count; ++s)
...
void NeonateVertexCompression_V4::deltaAndZigzagEncoding(std::vector<unsigned int>& input)
{
	size_t count = input.size();
...
void NeonateVertexCompression_V4::zigzagAndDeltaDecoding(std::vector<unsigned int>& input)
{
	size_t count = input.size();
	long long lastValue = 0;
	for (size_t s = 0; s < count; ++s)
...
int NeonateVertexCompression_V4::varintEncoding(const std::vector<unsigned int>& input, uint8_t* output)
{
	size_t outputSize = 0;
...
bool NeonateVertexCompression_V4::varintDecoding
(
	uint8_t*					input, 
	int							inputBufferSize, 
	int							elementCount, 
...
int NeonateVertexCompression_V4::uintEncoding
(
...
)
{
	...
	else if (precisionType == 2)
	{
		...
		NeonateVertexCompression_V4::deltaAndZigzagEncoding(lowPrecisionArray);
		outputLength = NeonateVertexCompression_V4::varintEncoding(lowPrecisionArray, output);
...
unsigned int NeonateVertexCompression_V4::positionEncoding
(
	std::vector<float>&				positions, 
	unsigned int&					outPosEncodingFlag,
	FBXSDK_NAMESPACE::FbxDouble3&	bboxMin, 
...
//void NeonateVertexCompression_V4::getSmoothingGroup()
//{
...
//void NeonateVertexCompression_V4::mergeTriangleStrips(osg::Geometry::PrimitiveSetList& primitives)
//{
//	int nbtristrip = 0;
...
void NeonateVertexCompression_V4::stripifyTriangles_Nvidia(const vector<unsigned int>& indices)
{
...
void NeonateVertexCompression_V4::stripifyTriangles_TriStripper(const vector<unsigned int>& indices)
{
...
int NeonateVertexCompression_V4::naiveTriangulate
(
	FbxMesh*			mesh,
	std::vector<int>&	triangleStartPerPolygon, 
	std::vector<int>&	triangleCountPerPolygon, 
...
void NeonateVertexCompression_V4::getTopologicalIndices(FbxMesh* mesh, std::vector<unsigned int>& topologyIndices)
{
...
int NeonateVertexCompression_V4::getPositionIndices
(
	FbxMesh*				mesh, 
	std::vector<int>&		indices, 
	std::vector<int>&		triangleCountPerPolygon, 
...
int NeonateVertexCompression_V4::getUVIndices
(
	FBXSDK_NAMESPACE::FbxMesh*	mesh, 
	std::vector<int>&			indices, 
	int							uvIndex,
...
int NeonateVertexCompression_V4::getNormalIndices
(
	FBXSDK_NAMESPACE::FbxMesh*	mesh, 
	std::vector<int>&			indices, 
	int							normalIndex, 
...
int NeonateVertexCompression_V4::getTangentIndices
(
	FBXSDK_NAMESPACE::FbxMesh*	mesh, 
	std::vector<int>&			indices, 
	int							tangentIndex, 
...
int NeonateVertexCompression_V4::getBinormalIndices
(
	FBXSDK_NAMESPACE::FbxMesh*	mesh,
	std::vector<int>&			indices,
	int							tangentIndex,
...
int NeonateVertexCompression_V4::getVertexColorIndices(FBXSDK_NAMESPACE::FbxMesh* mesh, std::vector<int>& indices, int vertexColorIndex, std::vector<int>& triangleCountPerPolygon, std::vector<int>& triangles)
{
...
FbxAMatrix NeonateVertexCompression_V4::getGeometryMatrix(FBXSDK_NAMESPACE::FbxNode* node)
{
...
FbxAMatrix NeonateVertexCompression_V4::getWorldToBoneLocalMatrixAtBindPose(FBXSDK_NAMESPACE::FbxCluster* cluster, FBXSDK_NAMESPACE::FbxNode* meshNode)
{
...
FbxVector4 NeonateVertexCompression_V4::transformPositionWithGeometricTransform(const FBXSDK_NAMESPACE::FbxAMatrix& meshGeometry, FbxDouble x, FbxDouble y, FbxDouble z)
{
...
FbxVector4 NeonateVertexCompression_V4::transformNormalWithGeometricTransform(const FBXSDK_NAMESPACE::FbxAMatrix& meshGeometry, FbxDouble x, FbxDouble y, FbxDouble z)
{
...
void NeonateVertexCompression_V4::getEffectiveBones(FbxMesh* mesh, std::vector<unsigned int>& effectiveBones)
{
	...
	for (int i = 0; i < clusterCount; ++i)
	{
		...
		effectiveBones.push_back((unsigned int)id);
	}
}

void NeonateVertexCompression_V4::getSkinning
(
	FbxMesh* mesh, 
...
)
{
	...
	for (int i = 0; i < effectiveBoneCount; ++i)
	{
		...
		bone.id = (unsigned int)node->GetUniqueID();
		bone.name = standardizeFileName(node->GetName());
		bone.modelWorldToBoneLocal = worldToBoneLocalMatrices[i];
		if (node->GetParent())bone.parentID = (unsigned int)node->GetParent()->GetUniqueID();
...
FBXSDK_NAMESPACE::FbxVector4* NeonateVertexCompression_V4::computeVertexNormals(FBXSDK_NAMESPACE::FbxVector4* positions, FBXSDK_NAMESPACE::FbxUInt32 positionCount, std::vector<int>& indices)
{
...
void NeonateVertexCompression_V4::getMorph(
	FBXSDK_NAMESPACE::FbxMesh* mesh, 
	...
	std::vector<MORPH_DATA_V2>* morphTargetsData)
{
	...
	mMeshDetail.blendshapeCount = blendShapeChannelCount;
		...
		if (channel)
		{
			...
			if (blendShape)
			{
				...
				if (positionCount > 0 && normalCount > 0)
				{
					...
					morphData.uniqueID = (unsigned int)uniqueID;
					...
					if (!identicalMatrix)
					{
						...
						{
							FBXSDK_NAMESPACE::FbxVector4 p = positions[i];
							positions[i] = NeonateVertexCompression_V4::transformPositionWithGeometricTransform(geometricMatrix, p[0], p[1], p[2]);							
						}
					}
					...
					for (unsigned int i = 0; i < vertexCount; ++i)
					{
						...
						if (!identicalMatrix)
						{
							n = NeonateVertexCompression_V4::transformNormalWithGeometricTransform(geometricMatrix, n[0], n[1], n[2]);
						}
						n.Normalize();
...
void NeonateVertexCompression_V4::createSubmeshByMaterials(FBXSDK_NAMESPACE::FbxMesh* mesh, const int materialCount, int layerIndex, std::vector<std::vector<int> >& subMeshes, \
...
bool NeonateVertexCompression_V4::appendNewLayer_V2(std::vector<std::vector<int>*>& vb, std::vector<int>& newLayerIndices, std::vector<int>& indices, VERTEX_LAYER_TYPE layerType)
{
		...
		else
		{
			...
			while (first != NULL)
			{
				...
				else
				{
					...
					else if (layerType == VL_UV1)
					{
						FBXSDK_NAMESPACE::FbxVector2 U0 = mUVArray1->GetAt(first->INDEX);
						FBXSDK_NAMESPACE::FbxVector2 U1 = mUVArray1->GetAt(newElementIndex);
						if (U0 == U1)
						{
							identical = true;
						}
...
const int NeonateVertexCompression_V4::mscVeretexCacheSize = 24;
const float NeonateVertexCompression_V4::mscPositionPrecision = 0.5f;
const double NeonateVertexCompression_V4::mscEpsilonDegree = 1.05;
const double NeonateVertexCompression_V4::mscEpsilon = mscEpsilonDegree*M_PI/180.0;
const unsigned int NeonateVertexCompression_V4::msc_N_phi = 120; //(int)ceil(MIN_N_phi); N_theta max is 248, total points 18778
const double NeonateVertexCompression_V4::msc_MIN_N_phi = M_PI / (2.0*mscEpsilon) + 1.0;
std::vector<unsigned int> NeonateVertexCompression_V4::ms_N_theta_series;
double NeonateVertexCompression_V4::msPhiDecodingFactor = 0.0;

void NeonateVertexCompression_V4::initNormalCompressor()
{
...
void NeonateVertexCompression_V4::mapNormal(
	double x, 
	double y, 
	double z,
	double w,
...
//		maxThetaCount = N_theta > maxThetaCount ? N_theta : maxThetaCount;
//		totalPointCount += N_theta;
//		NTHETA_SERIES.push_back(N_theta);
//	}
//
//	msPhiDecodingFactor = M_PI / (NTHETA_SERIES.size() - 1);
//}
...
//	double phi = acos(z);
//	double theta = atan2(y, x);
//	theta = theta < 0.0 ? (2.0 * PI + theta) : theta;
//	double j = round(phi * (NPHI - 1) / PI);
//	j -= (w * 128.0);
//	double k = round(theta * NTHETA_SERIES[(unsigned int)j] / (2.0 * PI));
//
//	encodedPhi = (unsigned int)j;
//	encodedTheta = (unsigned int)k;
//}

unsigned short NeonateVertexCompression_V4::RGB888_RGB565(unsigned char red, unsigned char green, unsigned char blue)
{
...
unsigned int NeonateVertexCompression_V4::ARGB8565_ARGB8888(unsigned short color, unsigned char alpha)
{
...
//unsigned short NeonateVertexCompression_V4::Float32ToFloat16(float f)
//{
//	unsigned int Result;
//
...
void NeonateVertexCompression_V4::Orthonormlize(
...
bool NeonateVertexCompression_V4::ComputeParity(
...
double NeonateVertexCompression_V4::ComputeTriangleArea(
...
void NeonateVertexCompression_V4::computeTangents_V2(
...
void NeonateVertexCompression_V4::computeTangents(
