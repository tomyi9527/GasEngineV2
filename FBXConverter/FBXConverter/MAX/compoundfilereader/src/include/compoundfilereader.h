...
public:
    ...
    const uint16_t* GetStringProperty(uint32_t propertyID)
    {
        for (uint32_t i = 0; i < m_hdr->NumProperties; i++)
        {
            if (m_hdr->propertyIdentifierAndOffset[i].id == propertyID)
            {
                ...
                if (data.size() < 8) throw FileCorrupted();
                uint32_t stringLengthInChar = helper::ParseUint32(data.data() + 4);
                if (data.size() < 8 + stringLengthInChar * 2) throw FileCorrupted();
                return reinterpret_cast<const uint16_t*>(data.data() + 8);
            }
        }
    ...
    int GetPropertyCount() const {
        return m_hdr ? m_hdr->NumProperties : 0;
    }
    uint32_t GetPropertyID(int index) const {
        return index >= GetPropertyCount() ? -1 : m_hdr->propertyIdentifierAndOffset[index].id;
    }
    const std::string_view GetPropertyData(int index) const {
        if (index >= GetPropertyCount()) {
            return std::string_view();
        }
        uint32_t start = m_hdr->propertyIdentifierAndOffset[index].offset;
        uint32_t end = m_bufferLen;
        if (index + 1 < m_hdr->NumProperties) {
            end = m_hdr->propertyIdentifierAndOffset[index + 1].offset;
        }
        return std::string_view((const char*)m_buffer + start, end - start);
    }
