...
// disable the 揻ormat not a string literal and no format arguments?warning since
// the FBXSDK_printf calls made here are all valid calls and there is no secuity risk
#pragma GCC diagnostic ignored "-Wformat-security"
#endif

void DisplayAnimation(FbxAnimStack* pAnimStack, FbxNode* node, bool isSwitcher = false);
void DisplayAnimation(FbxAnimLayer* pAnimLayer, FbxNode* node, bool isSwitcher = false);
void DisplayChannels(FbxNode* node, FbxAnimLayer* pAnimLayer, void (*DisplayCurve) (FbxAnimCurve* pCurve), void (*DisplayListCurve) (FbxAnimCurve* pCurve, FbxProperty* pProperty), bool isSwitcher);
...
void DisplayAnimation(FbxAnimStack* pAnimStack, FbxNode* node, bool isSwitcher)
{
    int l;
    ...
    for (l = 0; l < nbAnimLayers; l++)
    {
        ...
        DisplayAnimation(lAnimLayer, node, isSwitcher);
    }
}

void DisplayAnimation(FbxAnimLayer* pAnimLayer, FbxNode* node, bool isSwitcher)
{
    int lModelCount;
    FbxString lOutputString;
    ...
    DisplayChannels(node, pAnimLayer, DisplayCurveKeys, DisplayListCurveKeys, isSwitcher);
    FBXSDK_printf ("\n");

    for(lModelCount = 0; lModelCount < node->GetChildCount(); lModelCount++)
    {
        DisplayAnimation(pAnimLayer, node->GetChild(lModelCount), isSwitcher);
    }
}
